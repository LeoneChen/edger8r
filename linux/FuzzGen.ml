...
let get_fuzz_data_kind_str (data_kind : fuzz_data_kind) : string =
  ...
  | FUZZ_DATA -> "FUZZ_DATA"
  | FUZZ_ARRAY -> "FUZZ_ARRAY"
  | FUZZ_SIZE -> "FUZZ_SIZE"
  | FUZZ_COUNT -> "FUZZ_COUNT"
  | FUZZ_RET -> "FUZZ_RET"
  | FUZZ_BOOL_SET_NULL -> "FUZZ_BOOL_SET_NULL"
  | FUZZ_SEQ -> "FUZZ_SEQ"
  | FUZZ_P_DOUBLE -> "FUZZ_P_DOUBLE"
  | FUZZ_DATA_OR_PTR -> "FUZZ_DATA_OR_PTR"
...
let gen_basic_data (ty : atype) (var_access : string) (feed_data : bool)
  ...
  if feed_data then
    Printf.sprintf "%sDFGetBytes(&%s, sizeof(%s), \"\", %s);\n"
      (get_indent indent_cnt) var_access (Ast.get_tystr ty)
...
let gen_ptr_data (var_access : string) (count_var : string) (elety : atype)
    ...
    =
  if feed_data then
    Printf.sprintf "%sDFGetBytes(%s, %s * sizeof(%s), \"\", %s);\n"
      (get_indent indent_cnt) var_access count_var (Ast.get_tystr elety)
      (get_fuzz_data_kind_str data_kind)
  else ""
...
let gen_ptr_data2 (var_access : string) (count_var : string)
  ...
  if feed_data then
    Printf.sprintf "%sDFGetBytes(%s, %s * sizeof(%s), \"\", FUZZ_DATA);\n"
      (get_indent indent_cnt) var_access count_var elem_tystr
  else ""
            ...
            | Ptr elety ->
                ...
                if attr.pa_isstr then
                  ...
                  let count_stat =
                    ...
                    then
                      Printf.sprintf
                        "%ssize_t %s = DFGetUserCheckCount(sizeof(%s), \"\");\n"
                        (get_indent (depth + 1))
                        count_var elem_tystr
                    else
                      ...
                      let size_tag =
                        ...
                        | None -> Printf.sprintf "sizeof(%s)" elem_tystr
                      in
                      if is_ecall then
                        Printf.sprintf
                          "%ssize_t %s = ((%s) * (%s) + sizeof(%s) - 1 ) / \
                           sizeof(%s);\n"
                          (get_indent (depth + 1))
                          count_var count_tag size_tag elem_tystr elem_tystr
                      else
                        Printf.sprintf
                          "%ssize_t %s = ((%s) * (%s)) / sizeof(%s);\n"
                          (get_indent (depth + 1))
                          count_var count_tag size_tag elem_tystr
                  in
                  ...
                  let get_buf_stat =
                    if is_ecall then
                      Printf.sprintf
                        "%s%s = (%s)DFManagedCalloc(%s, sizeof(%s));\n"
                        (get_indent (depth + 1))
                        inner_var_access tystr count_var elem_tystr
                    else ""
                  in
                  ...
                  let count_stat =
                    ...
                    then
                      ...
                      let size_tag =
                        ...
                        | None -> Printf.sprintf "sizeof(%s)" var_deref_access
                      in
                      if is_ecall then
                        Printf.sprintf
                          "%ssize_t %s = ((%s) * (%s) + sizeof(%s) - 1 ) / \
                           sizeof(%s);\n"
                          (get_indent (depth + 1))
                          count_var count_tag size_tag var_deref_access
                          var_deref_access
                      else
                        Printf.sprintf
                          "%ssize_t %s = ((%s) * (%s)) / sizeof(%s);\n"
                  ...
                  let get_buf_stat =
                    if is_ecall then
                      Printf.sprintf
                        "%s%s = (%s)DFManagedCalloc(%s, sizeof(%s));\n"
                        (get_indent (depth + 1))
                        inner_var_access tystr count_var var_deref_access
                    else ""
                  in
                  let get_bytes_stat =
...
let gen_fuzzing_code (ec : enclave_content) (dirs : string list) :
  ...
  let header =
    "\n\
     ...
     extern uint8_t* DFGetBytes(void* ptr, size_t byteArrLen, char* \
     cStrAsParamID, enum FuzzDataTy dataType);\n\
     extern size_t DFGetUserCheckCount(size_t eleSize, char *cStrAsParamID);\n\
     extern void *DFManagedCalloc(size_t count, size_t size);\n\
     extern int DFEnableSetNull(char *cStrAsParamID);\n\
     extern int DFEnableModifyOCallRet(char *cParamID);\n\
