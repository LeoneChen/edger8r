...
let gen_ecall_fuzz_wrapper (tf : trusted_func) : string =
  ...
  let call_ecall =
    Printf.sprintf
      "auto ret=%s(__g_harness_eid%s%s);\n\
       if (ret!=SGX_SUCCESS && ret!=SGX_ERROR_INVALID_PARAMETER && ret!= \
       SGX_ERROR_ECALL_NOT_ALLOWED){\n\
       fprintf(stderr,\"%s returned with error code: 0x%%X\\n\",ret);\n\
       abort();\n\
       }"
      fd.fname ret_param param_list_str fd.fname
  in
